...
// Load all selected_activities + events data
const loadData = async () => {
	// output key  = selected_activities
	try {
		let data = await axios.get(
			process.env.REACT_APP_BACKEND_URL + "/api/admin/selected-activities",
			{
				headers: {
...
const addSpotlight = async ({ title }) => {
	...
	try {
		console.log(title);
		await axios.post(
			process.env.REACT_APP_BACKEND_URL + "/api/admin/selected-activity/create",
			{
				title: title,
			},
			{
...
const updateSpotlight = async ({ spotlightId, title }) => {
	// output key  = selected_activity
	console.log(title);
	try {
		await axios.post(
			process.env.REACT_APP_BACKEND_URL + "/api/admin/selected-activity/update",
			{
				selected_activity_id: spotlightId,
				title: title,
				_method: "PUT",
			},
			{
...
const delSpotlight = async ({ spotlightId }) => {
	...
	try {
		await axios.post(
			process.env.REACT_APP_BACKEND_URL + "/api/admin/selected-activity/delete",
			{
				selected_activity_id: spotlightId,
				_method: "DELETE",
			},
	...
	try {
		await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/admin/selected-activity/set-view",
			{
				selected_activity_id: spotlightId,
			},
			{
...
const getSpotlightData = async ({ spotlightId }) => {
	// output key  = selected_activity (main)
	try {
		let data = await axios.get(
			process.env.REACT_APP_BACKEND_URL +
				"/api/selected-activity?selected_activity_id=" +
				spotlightId,
			{
				headers: {
					"x-api-key": process.env.REACT_APP_BACKEND_KEY,
...
const addEventSpotlight = async ({ spotlightId, eventId }) => {
	...
	try {
		await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/admin/selected-activity/event/add",
			{
				selected_activity_id: spotlightId,
				event_id: eventId,
			},
...
const toLeftInSpotlight = async ({ spotlightEventId }) => {
	...
	try {
		let data = await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/admin/selected-activity/event/set-prio-plus",
			{
				selected_activity_data_id: spotlightEventId,
			},
			{
				headers: {
					Authorization: "Bearer " + localStorage.getItem("access_token"),
...
const toRightInSpotlight = async ({ spotlightEventId }) => {
	...
	try {
		let data = await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/admin/selected-activity/event/set-prio-min",
			{
				selected_activity_data_id: spotlightEventId,
			},
			{
...
const delDataInSpotlight = async ({ spotlightEventId }) => {
	...
	try {
		let data = await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/admin/selected-activity/event/delete",
			{
				selected_activity_data_id: spotlightEventId,
				_method: "DELETE",
			},
...
const SelectedActivities = ({
			...
			if (statePosOpenPopUp === "add") {
				addSpotlight({
					...
					if (res.status === 200) {
						...
						setData(res.data.selected_activities);
						resetForm();
						setAlert({
							state: true,
	...
	const handleNavLeft = (id, evtSpotId) => {
		...
		toLeftInSpotlight({ spotlightEventId: evtSpotId }).then((res) => {
			if (res.status === 202) {
				...
				showData.forEach((data) => {
					if (data.data.id === id) {
						showData[index].events = res.data.selected_activity_datas;
					}
					index++;
				});
				setAlert({
	...
	useEffect(() => {
		fnSetActive("selected-activities");
		if (showData === null) {
			setLoading(true);
			loadData().then((res) => {
				setLoading(false);
				...
				<div className="modal-dialog modal-dialog-centered">
					<div className="modal-content">
						<form onSubmit={handleAddEvent}>
							<div className="row m-0">
								<div className="col-12 d-flex pt-4 ps-3 pe-3 pb-1">
									<h5>Add Activity In Selected Group</h5>
									<button
										type="button"
										className="btn-close ms-auto"
										data-bs-dismiss="modal"
								...
								showData.length !== 0 ? (
									...
									>
										...
										<p className="fw-bold fs-5 text-center mb-auto mt-3">
											No Selected Activities Group Data
										</p>
									</div>
								)
							) : (
